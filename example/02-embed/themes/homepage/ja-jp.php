...
<body>
	...
	<h2>02-embed</h2>
	<h3>内部の読み込みファイル &amp; 埋め込み</h3>
	<p>自動的に検索の既定ファイルを選択するディレクトリの下の一致するファイルを検索しないでくださいと呼ばれます。</p>

	<h2>構成情報</h2>
	<ol>
		<li>現在 Cookies グローバル 'lang' 変数の値: <?php if(isset($_COOKIE["lang"])) echo $_COOKIE['lang']; else echo "設定されていません。"; ?></li>
		<li>ブラウザー 'HTTP_ACCEPT_LANGUAGE' 要求値: <?php preg_match('/^([a-z\-]+)/i', $_SERVER['HTTP_ACCEPT_LANGUAGE'], $matches); echo $matches[1]; ?></li>
	...
	<h2>それを使用する方法</h2>
	<ol>
		<li>クイックガイドを読む -> <a href="https://lang-php.com/go/lp-docs-two">https://lang-php.com/go/lp-docs-two</a></li>
		<li>ウィル 'example/02-embed' ディレクトリ内のすべてのファイルをWebサイトディレクトリにアップロードします；</li>
